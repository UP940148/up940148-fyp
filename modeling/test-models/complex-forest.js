...
export default async function createScene() {
  ...
  let treeData;
  try {
    const treeResponse = await fetch('../modeling/test-models/poisson-samples/r35.json');
    treeData = await treeResponse.json();
  } catch {
    treeData = retrieve('../modeling/test-models/poisson-samples/r35.json');
  } finally {
    let t = 0;
    while (t < treeData.length) {
      const tree = await TreeLoader.load('../modeling/trees/r30N8000t30tp1_9.json', true);
