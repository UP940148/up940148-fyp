...
export default async function createScene() {
  ...
  let treeData;
  try {
    const treeResponse = await fetch('../modeling/test-models/poisson-samples/r30.json');
    treeData = await treeResponse.json();
  } catch {
    treeData = retrieve('../modeling/test-models/poisson-samples/r30.json');
  } finally {
    let t = 0;
    while (t < treeData.length) {
      const tree = await TreeLoader.load('../modeling/trees/r60N10000dk12.json', true, true);
      // const tree = makeCube(cubeLightReflectance, cubeLightEmittance);
      const transform = new Transform3();
      // Center on 0, 0
      transform.translate(-0.5, -0.5, 0);
      transform.scale(0.25, 0.25, 0.35);
